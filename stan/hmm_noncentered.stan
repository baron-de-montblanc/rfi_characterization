...
transformed parameters {
  ...
  for (m in 1:M) {
    ...
    X[m][1] = mu0 + scale_bf * X_raw[m][1];
    for (l in 2:L)
      X[m][l] = loc_X[l-1] + scale_X[l-1] * X_raw[m][l];
  }
...
model {
  ...
  for (m in 1:M) {
    target += generalized_normal_lpdf(X_raw[m][1] | 0, 1, shape_bf);
    for (l in 2:L)
      target += generalized_normal_lpdf(X_raw[m][l] | 0, 1, shape_X);
  }
